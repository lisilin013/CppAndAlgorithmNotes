...
BOOL InsertElem(pstLinkList pstll, int index, ElementType elem)
{
	...
	if(GetptrList(pstll, index-1, &p))
	{
		...
		if(p->next)//in case insert at the end
			p->next->prior = pNode;
		p->next = pNode;
		return TRUE;
	}
...
BOOL DeletElem(pstLinkList pstll, int index)
{
	...
	if(GetptrList(pstll, index-1, &p))
	{
		...
		if(pDelNode->next)//in case pDelnode is not the last node
			pDelNode->next->prior = p;//diff
		free(pDelNode);
		printf("free ok!\n");
		return TRUE;
	}
...
BOOL GetPriorElem(pstLinkList pstll, ElementType * elem)
{
	if(pstll->prior)
	{
		*elem = pstll->prior->data;
		return TRUE;
	}
	return FALSE;
}
