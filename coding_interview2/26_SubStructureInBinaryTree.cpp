//
// Created by Silin Li on 09:02 02/08/2020.
// 
...
using BinaryTreeNode = BinaryTreeNode<int>;
using BinaryTree = BinaryTree<int>;

/**
 * @brief root1根节点的树是否包含root2根节点的树，root1要和root2一一对应
 * 其中root1==root2
 */
bool DoesTree1HasTree2BasedOnRoot(BinaryTreeNode *root1, BinaryTreeNode *root2) {
    if (!root2)
        return true;
    if (!root1)
        return false;
    ...
    return DoesTree1HasTree2BasedOnRoot(root1->left, root2->left) &&
           DoesTree1HasTree2BasedOnRoot(root1->right, root2->right);
}

/**
 * @brief root1是否包含root2结构
 */
bool HasSubTree(BinaryTreeNode *root1, BinaryTreeNode *root2) {
    ...
    // 1.在root1中搜索到和root2根节点值相同的节点返回
    bool result = false;
    if (root1->data == root2->data) {
        result = DoesTree1HasTree2BasedOnRoot(root1, root2);
    }
    if (!result)
